...
Loom includes a built-in long-term memory system that lets models remember
...
└─────────────────────────────┘
        ...
        ▼
┌─────────────────────────────┐
│   Loom Model              │  ← sees personalised context
│   (inference)               │
└─────────────────────────────┘
        │
        ▼
...
The memory system uses Loom's own `/api/embed` endpoint:

```bash
loom pull nomic-embed-text
```

Any embedding model works. `nomic-embed-text` is recommended for its speed and
accuracy at 768 dimensions.

### 2. Enable memory in `~/.loom/server.json`
...
All options live under the `"memory"` key in `~/.loom/server.json`.
...
| `db_path` | `~/.loom/memory.lance` | Path to the LanceDB database directory. |
| `embedding_model` | `nomic-embed-text` | Loom model used to generate embeddings. |
...
Memory is stored in a LanceDB database directory (default `~/.loom/memory.lance`):
...
- All memories are stored **locally** in `~/.loom/memory.db`
- Embeddings are generated by your local Loom server — nothing is sent to the cloud
- The memory system is **off by default** (`"enabled": false`)
- To clear all memories: `rm ~/.loom/memory.db`
