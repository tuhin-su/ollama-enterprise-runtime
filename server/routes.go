...
import (
	...
	"github.com/loom/loom/tools"
	"github.com/loom/loom/types/errtypes"
	"github.com/loom/loom/types/model"
	"github.com/loom/loom/version"
	imagegenmanifest "github.com/loom/loom/x/imagegen/manifest"
	xserver "github.com/loom/loom/x/server"
)

const signinURLStr = "https://loom.com/connect?name=%s&key=%s"
...
func experimentEnabled(name string) bool {
	return slices.Contains(strings.Split(os.Getenv("LOOM_EXPERIMENT"), ","), name)
}
...
func (s *Server) scheduleRunner(ctx context.Context, name string, caps []model.Capability, requestOpts map[string]any, keepAlive *api.Duration, shift *bool) (llm.LlamaServer, *Model, *api.Options, error) {
	...
	if slices.Contains(model.Config.ModelFamilies, "mllama") && len(model.ProjectorPaths) > 0 {
		return nil, nil, nil, fmt.Errorf("'llama3.2-vision' is no longer compatible with your version of Loom and has been replaced by a newer version. To re-download, run 'loom pull llama3.2-vision'")
	}
...
func (s *Server) GenerateHandler(c *gin.Context) {
	...
	if !req.Raw {
		...
		var b bytes.Buffer
		if req.Context != nil {
			slog.Warn("the context field is deprecated and will be removed in a future version of Loom")
...
func GetModelInfo(req api.ShowRequest) (*api.ShowResponse, error) {
	...
	var sb strings.Builder
	fmt.Fprintln(&sb, "# Modelfile generated by \"loom show\"")
	modelfile := m.String()
	if m.IsMLX() {
...
// authentication when an api_token is configured in ~/.loom/server.json.
...
func (s *Server) GenerateRoutes() (http.Handler, error) {
	...
	// General
	r.HEAD("/", func(c *gin.Context) { c.String(http.StatusOK, "Loom is running") })
...
func Serve(ln net.Listener) error {
	...
	slog.Info(fmt.Sprintf("Loom cloud disabled: %t", cloudDisabled))
	...
	if useClient2 {
		slog.Warn("LOOM_EXPERIMENT=client2 is no longer available. Please remove this environment.")
	}
	...
	// Memory subsystem — reads ~/.loom/server.json for config.
	...
	// Task scheduler — always enabled, persists jobs to ~/.loom/scheduler.json.
...
func (s *Server) WhoamiHandler(c *gin.Context) {
	...
	u, err := url.Parse("https://loom.com")
	if err != nil {
...
func chatModeForModel(m *Model) chatExecutionMode {
	if m.IsMLX() || usesLoomRenderedChat(m) {
...
func llamaServerConfigForModel(m *Model) llm.LlamaServerConfig {
	return llm.LlamaServerConfig{
		DisableJinja:   usesLoomRenderedChat(m),
		DraftModelPath: m.DraftPath,
	}
}

func usesLoomRenderedChat(m *Model) bool {
...
func (s *Server) ChatHandler(c *gin.Context) {
	...
	if toolHeader != "" || userHeader != "" {
		slog.Info("Loom client context received", "user", userHeader, "tool", toolHeader)
	}
	...
	// Stream chat event to RabbitMQ if enabled
	GetRabbitMQPublisher().PublishEvent("chat_request", req.Model, map[string]any{
...
// AnthropicMessagesHandler proxy-translates Anthropic /v1/messages requests to Loom
func (s *Server) AnthropicMessagesHandler(c *gin.Context) {
	...
	var loomMsgs []api.Message
	if systemStr != "" {
		loomMsgs = append(loomMsgs, api.Message{
			Role:    "system",
			Content: systemStr,
		})
	}
